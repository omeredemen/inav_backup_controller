...
static bool handleIncoming_SYS_STATUS(void) {
    mavlink_sys_status_t msg;
    mavlink_msg_sys_status_decode(&mavRecvMsg, &msg);

    /* if SYS_STATUS is used in the system add code below*/
    return true;
}
        ...
        if (result == MAVLINK_FRAMING_OK) {
            switch (mavRecvMsg.msgid) {
                ...
                case MAVLINK_MSG_ID_SYS_STATUS:
                    return handleIncoming_SYS_STATUS();
